...
the `rsi 2-period strategy` is configured by the following parameters:

### rsi period
-   **definition**: the number of periods used to calculate the relative strength index (rsi).
-   **significance**: determines the sensitivity of the rsi to price changes. for this strategy, it is fixed at 2.
-   **impact**:
    -   **shorter period (e.g., 2)**: extremely sensitive, generating many overbought/oversold signals, often used for very short-term trading.
    -   **longer period (e.g., 14)**: less sensitive, fewer signals, better for identifying broader overbought/oversold conditions.

### oversold threshold
-   **definition**: the rsi value below which an asset is considered oversold, potentially signaling a buying opportunity.
-   **significance**: defines the lower bound for entry signals.
-   **impact**:
    -   **lower threshold (e.g., 5)**: fewer, stronger buy signals.
    -   **higher threshold (e.g., 15)**: more frequent, weaker buy signals.

### overbought threshold
-   **definition**: the rsi value above which an asset is considered overbought, potentially signaling a selling opportunity.
-   **significance**: defines the upper bound for exit signals.
-   **impact**:
    -   **higher threshold (e.g., 95)**: fewer, stronger sell signals.
    -   **lower threshold (e.g., 85)**: more frequent, weaker sell signals.
